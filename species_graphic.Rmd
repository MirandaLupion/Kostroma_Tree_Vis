...
mutate(y_pos = (case_when(key == "aspen" ~ 100,
                        key == "birch" ~  98,
                        key == "birch_and_alder" ~ 95,
                      key == "birch_and_aspen" ~ 75,
                      key == "pine" ~ 65))) %>%
mutate(x_pos = (case_when(key == "aspen" ~ 1.5,
                          key == "birch" ~ 1.5,
                         key == "birch_and_alder" ~ 1.48,
                         key == "birch_and_aspen" ~ 1.48,
                         key == "pine" ~ 1.48)))  
...
pie_chart <- pie_chart + geom_text(label = pie_buysky$value, x = pie_buysky$x_pos, y = pie_buysky$y_pos) 
 

pie_chart
#  geom_text(label = "31%", y = 84.5, x = 1.2) +
#   
#   geom_text(aes(x = 1.5, y = pos, label = paste0(round(value), "%")),
#                   # hjust = 1.2,
